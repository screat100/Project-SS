...
public class ShootBehaviour : GenericBehaviour
{
    ...
    void Start()
	{
		...
		if (!neck)
		{
			neck = animator.GetBoneTransform(HumanBodyBones.Head).parent;
		}
	...
	private void Update()
	{
        ...
        isAiming=animator.GetBool(aimBool);
	...
	{
		isAimBlocked = Physics.SphereCast(this.transform.position + castRelativeOrigin, 0.1f, transform.forward, out RaycastHit hit, distToHand - 0.1f);
		...
		Debug.DrawRay(this.transform.position + castRelativeOrigin, transform.forward * distToHand, isAimBlocked ? Color.red : Color.cyan);
	...
	public void OnAnimatorIK(int layerIndex)
	{
		if (isAiming && activeWeapon > 0)
		{
			...
			float xCamRot = Quaternion.LookRotation(transform.forward).eulerAngles.x;
